...
print(nrow(sentences_df))
# Preallocate space for pairs that need to be compared 
comparison_pairs_n_est <- (nrow(sentences_df) ^ 2) * 0.0011 * 1.05
comparison_pairs_df <- data.frame(query_id=rep(-1, comparison_pairs_n_est),
                                  compare_id=rep(-1, comparison_pairs_n_est),
                                  is.common_query_word_1=rep(FALSE, 
                                                             comparison_pairs_n_est))
...
myprint_df(subset(comparison_pairs_old_df, query_id == 4805), dims=TRUE)
myprint_df(subset(comparison_pairs_df, query_id == 4805), dims=TRUE)
tmp_df <- merge(subset(comparison_pairs_df, query_id == 4805), 
                subset(comparison_pairs_old_df, query_id == 4805))
myprint_df(tmp_df, dims=TRUE)

# print(all.equal(subset(comparison_pairs_df, select=-c(is.common_query_word_1)), 
#                 comparison_pairs_old_df))
comparison_pairs_tmp_df <- merge(comparison_pairs_df, 
                                  comparison_pairs_old_df)
comparison_pairs_tmp_df <- orderBy(~ query_id + compare_id, 
                                    data=comparison_pairs_tmp_df)
print(all.equal(comparison_pairs_tmp_df, 
                comparison_pairs_df))
...
### shd be (query_id=4593, compare_id=1748)
###     debug code
