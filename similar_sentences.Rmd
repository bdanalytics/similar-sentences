...
#for (loop_step_id in loop_df$loop_step_id[seq(2, 3)]) {   
    ptm <- proc.time()
...
for (loop_step_id in loop_df$loop_step_id) {
#for (loop_step_id in loop_df$loop_step_id[6:6]) {   
    ptm <- proc.time()
    ptm_diff <- proc.time() - ptm

    comparison_pairs_n_est <- (nrow(sentences_df) ^ 2) * 0.0014 * 1.05
    ...
    for (row_pos in seq(1:nrow(sentences_df))) {    
        ...
        if ((toString(loop_step_id) >= toString(loop_step_id_set[6])) & 
            (nrow(query_pairs_df) > 0)) {            
            #comparison_pairs_df <- rbind(comparison_pairs_df, query_pairs_df)
            comparison_pairs_df[(comparison_pairs_urow + 1):
                                (comparison_pairs_urow + nrow(query_pairs_df)),
                               ] <- query_pairs_df
            comparison_pairs_urow <- comparison_pairs_urow + nrow(query_pairs_df) 
        }
    }
...
print(all.equal(comparison_pairs_df, 
                comparison_pairs_old_df[, c("query_id", "compare_id")]))
#print(loop_df)
loop_steps_plot_df <- melt(loop_df, id="loop_step_id", 
                           measure=c("user", "system", "elapsed"))
p <- ggplot(data=loop_steps_plot_df, mapping=aes(x=loop_step_id, y=value)) + 
